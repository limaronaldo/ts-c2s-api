...
while (allLeads.size < 500) {
  try {
    ...
    if (leads.length === 0) {
      console.log(`   Página ${page}: 0 leads - fim dos dados`);
      break;
    }
    ...
    console.log(`   Página ${page}: ${leads.length} leads (${newCount} novos, total: ${allLeads.size})`);

    if (leads.length < perPage) {
      console.log(`   Página incompleta - fim dos dados`);
      break;
    }
...
console.log(`\n📊 Total de leads: ${leadsArray.length}\n`);
...
// Save CSV
const csvPath = '/Users/ronaldo/Projects/MBRAS/tools/ts-c2s-api/exports/all-leads-500.csv';
...
if (tainara) {
  ...
  const leadIds = new Set(data.leads.map((l: any) => l.id));

  for (let i = 0; i < leadsArray.length; i++) {
    if (leadIds.has(leadsArray[i].id)) {
  ...
  console.log(`✅ CSV da ${name} salvo em: ${tainaraCsvPath}`);
