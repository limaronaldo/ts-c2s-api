...
import { C2SService, type C2SCustomerAttributes } from './c2s.service'
import { DbStorageService } from './db-storage.service'
import { recentCpfCache, processingLeadsCache } from '../utils/cache'
import { normalizeIncome, normalizeName, normalizeEmail, normalizeCpf } from '../utils/normalize'
import { normalizePhone, formatPhoneWithCountryCode } from '../utils/phone'
import { buildDescription, buildSimpleDescription } from '../utils/description-builder'
...
export class EnrichmentService {
  ...
  async enrichLead(lead: LeadData): Promise<EnrichmentResult> {
    ...
    enrichmentLogger.info({ leadId, name, phone, email }, 'Starting lead enrichment')
    ...
    try {
      ...
      const cpf = await this.cpfDiscoveryService.findCpf(normalizedPhone, normalizedEmail ?? undefined)

      if (!cpf) {
        enrichmentLogger.info({ leadId }, 'Could not discover CPF, creating unenriched customer')
        return this.createUnenrichedCustomer(lead)
      }

      // Check if CPF was recently processed
      if (recentCpfCache.has(cpf)) {
        enrichmentLogger.info({ leadId, cpf }, 'CPF was recently processed, skipping')
        return {
          success: true,
          cpf,
          enriched: false,
      ...
      if (!personData) {
        enrichmentLogger.info({ leadId, cpf }, 'No enrichment data found, creating basic customer')
      ...
      // Update lead status
      await this.dbStorage.updateLeadEnrichmentStatus(leadId, 'completed', party.id, c2sResult.data.id)

      enrichmentLogger.info({ leadId, cpf, c2sCustomerId: c2sResult.data.id }, 'Lead enrichment completed')
  ...
  private async storePartyData(person: WorkApiPerson, cpf: string): Promise<{ id: string }> {
    // Create/update party
  ...
  private async createEnrichedCustomer(lead: LeadData, person: WorkApiPerson) {
    ...
    const attributes: C2SCustomerAttributes = {
      name: normalizeName(person.nome) || lead.name,
    ...
    return this.c2sService.createCustomer(attributes)
  }

  private async createUnenrichedCustomer(lead: LeadData): Promise<EnrichmentResult> {
    const description = buildSimpleDescription(lead.name, lead.phone, lead.email, lead.campaignName)

    const attributes: C2SCustomerAttributes = {
      name: normalizeName(lead.name) || lead.name,
      phone: lead.phone ? formatPhoneWithCountryCode(lead.phone) : undefined,
      email: lead.email ? normalizeEmail(lead.email) ?? undefined : undefined,
      description,
      source: lead.source || 'google_ads',
    ...
    const result = await this.c2sService.createCustomer(attributes)

    await this.dbStorage.updateLeadEnrichmentStatus(lead.leadId, 'unenriched', undefined, result.data.id)
  ...
  private async createBasicCustomer(lead: LeadData, cpf: string): Promise<EnrichmentResult> {
    const description = buildSimpleDescription(lead.name, lead.phone, lead.email, lead.campaignName)
    ...
    await this.dbStorage.updateLeadEnrichmentStatus(lead.leadId, 'basic', undefined, result.data.id)
