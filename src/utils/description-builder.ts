 ...
 */
export function buildDescription(person: WorkApiPerson, campaignName?: string): string {
  const lines: string[] = []
  ...
  if (person.renda || person.rendaPresumida) {
    ...
    if (person.renda) {
      lines.push(`Renda: R$ ${person.renda.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`)
    }
    if (person.rendaPresumida) {
      lines.push(`Renda Presumida: R$ ${person.rendaPresumida.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`)
    }
    if (person.patrimonio) {
      lines.push(`Patrimônio: R$ ${person.patrimonio.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`)
    }
    lines.push('')
  }
  ...
  if (person.enderecos && person.enderecos.length > 0) {
    ...
    for (const addr of person.enderecos.slice(0, 2)) {
      const parts = [addr.logradouro, addr.numero, addr.complemento, addr.bairro, addr.cidade, addr.uf, addr.cep].filter(
        Boolean
      )
 ...
 */
export function buildSimpleDescription(name: string, phone?: string, email?: string, campaignName?: string): string {
  const lines: string[] = []
