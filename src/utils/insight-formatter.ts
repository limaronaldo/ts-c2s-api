...
function formatSingleInsight(insight: LeadInsight): string {
  ...
  if (insight.sources && insight.sources.length > 0) {
    lines.push(`   📎 Fonte: ${insight.sources.join(', ')}`);
  }
 ...
 */
export function formatInsightMessage(insights: LeadInsight[], context: InsightContext): string {
  ...
  lines.push(`${tierEmoji} INSIGHT AUTOMÁTICO`);
  lines.push('');

  // Insights agrupados
  lines.push('📊 Perfil Descoberto:');
  ...
  if (context.income || context.propertyCount || context.addressCount) {
    lines.push('💰 Indicadores:');

    if (context.income) {
      lines.push(`   • Renda: ${formatCurrency(context.income)}/mês`);
    }

    if (context.propertyCount && context.propertyCount > 0) {
      lines.push(`   • Imóveis: ${context.propertyCount} registrados`);
    }

    if (context.addressCount && context.addressCount > 0) {
      lines.push(`   • Endereços: ${context.addressCount} encontrados`);
    }
  ...
  const primaryRecommendation = insights.find((i) => i.recommendation)?.recommendation;
  if (primaryRecommendation) {
    lines.push('🎯 Recomendação:');
    lines.push(`   ${primaryRecommendation}`);
    lines.push('');
  }
  ...
  lines.push(`⚡ Confiança: ${avgConfidence}%`);
 ...
 */
export function createHighIncomeInsight(income: number, presumedIncome?: number): LeadInsight {
  ...
  return {
    ...
    details,
    recommendation: income >= 20000 ? 'Lead prioritário. Atendimento VIP.' : undefined,
  };
}
 ...
 */
export function createRareSurnameInsight(surname: string, context?: string): LeadInsight {
...
export function createNotableFamilyInsight(
  ...
  return {
    ...
    recommendation: 'LEAD PRIORITÁRIO! Família de alto perfil. Atendimento especial.',
  };
}
 ...
 */
export function createInternationalInsight(country: string, phone: string): LeadInsight {
  return {
    type: 'international',
    confidence: 90,
 ...
 */
export function createMultiplePropertiesInsight(count: number, locations?: string[]): LeadInsight {
  const details = [`${count} imóveis registrados no CPF`];

  if (locations && locations.length > 0) {
    details.push(`Localizações: ${locations.slice(0, 3).join(', ')}${locations.length > 3 ? '...' : ''}`);
  }
...
export function createWebSearchInsight(
  ...
  if (companies && companies.length > 0) {
    details.push(`Empresas mencionadas: ${companies.join(', ')}`);
  }

  if (newsArticles && newsArticles.length > 0) {
    for (const article of newsArticles.slice(0, 2)) {
      details.push(`📰 ${article.source}: ${article.title.substring(0, 60)}...`);
    }
  }

  if (legalMentions && legalMentions.length > 0) {
    details.push(`📋 ${legalMentions.length} registro(s) público(s) encontrado(s)`);
  }
  ...
  return {
    ...
    recommendation: summary || 'Informações públicas encontradas na web.',
  };
}
