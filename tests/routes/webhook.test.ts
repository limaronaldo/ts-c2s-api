...
describe("POST /webhook/c2s", () => {
  ...
  test("webhook secret validation logic", () => {
    const configuredSecret: string = "my-secret-key";
    const incomingSignature: string = "my-secret-key";
  ...
  test("rejects invalid webhook signature", () => {
    const configuredSecret: string = "my-secret-key";
    const invalidSignature: string = "wrong-key";

    const isValid = configuredSecret === invalidSignature;
    expect(isValid).toBe(false);
  });
});
...
describe("POST /webhook/google-ads", () => {
  ...
  test("validates webhook key", () => {
    const configuredKey: string = "google-ads-webhook-key";
    const validKey: string = "google-ads-webhook-key";
    const invalidKey: string = "wrong-key";

    // Valid key
    expect(validKey === configuredKey).toBe(true);

    // Invalid key
    expect(invalidKey === configuredKey).toBe(false);
  });
