...
describe("surname-analyzer", () => {
  ...
  describe("detectFamilyConnection", () => {
    ...
    test("returns none for same person (exact match, different case)", () => {
      const result = detectFamilyConnection(
        "Ana Maria Pereira Ribeiro",
        "ANA MARIA PEREIRA RIBEIRO",
      );
      expect(result.type).toBe("none");
      expect(result.explanation).toContain("Mesma pessoa");
    });

    test("returns none for same person (with accents)", () => {
      const result = detectFamilyConnection("José da Silva", "JOSE DA SILVA");
      expect(result.type).toBe("none");
    });

    test("returns none for same person (partial match - all words present)", () => {
      const result = detectFamilyConnection("Ana Maria", "Ana Maria Pereira");
      expect(result.type).toBe("none");
    });
  });
